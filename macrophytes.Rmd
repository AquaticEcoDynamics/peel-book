...
::: {.body-text width="100%"}
The current macrophyte community was sampled in both spring 2017 and autumn 2018 at all 51 sites shown in Fig. \@ref(fig:macrophytes-pic4). Several new sites were added to this sampling regime to improve coverage in certain sections of the estuary (see sites marked on Fig. \@ref(fig:macrophytes-pic3)). The field and lab methods were the same as those described in section 7.3.1.2, except that each macrophyte taxa was identified to species rather than broad phyla or division level as in the previous surveys (Fig \@ref(fig:macrophytes-pic3)).

In contrast to the long-term macrophyte data set, the data analysis approach for the current data set focused equally on exploring the nature and extent of any differences in species biomass composition among estuarine *regions, water depths* and *seasons*. The same software as described in section 7.3.1.3 were used for these analyses. Full details of these analyses are given in @krumholz2019macrophyte.
:::
...
::: {.body-text width="100%"}
Significant changes have occurred in the macrophyte community over the last 40 years (Fig. \@ref(fig:macrophytes-pic6); [Supplementary Materials S7.1][Supplementary Materials]). The extent of these interannual period changes has been substantial, especially in autumn when they were more influential than any macrophyte changes due to differences in estuary regions or water depth.
...
Inter-period shifts in the macrophyte community are shown in more detail for each region of the estuary in the 'shadeplots' in Fig. \@ref(fig:macrophytes-pic7) (simply the darker the shading, the greater the biomass). Green algal and seagrass biomass are also interpolated across the whole estuary (based on site measurements) in Figs \@ref(fig:macrophytes-pic8) and \@ref(fig:macrophytes-pic9). In general, these plots show that reductions in green macroalgae over time have been most marked in Peel Inlet (in both the western and eastern sides in autumn and the western side in spring), while seagrass has increased most obviously in the northern Harvey, followed by Peel Inlet (Figs \@ref(fig:macrophytes-pic7)--\@ref(fig:macrophytes-pic9)).
...
Another (but lesser) exception to the general long-term trends across most of the estuary was the eastern Peel Inlet, especially in spring, where green algal biomass is still comparatively high (Figs \@ref(fig:macrophytes-pic8), \@ref(fig:macrophytes-pic9)). Growths are particularly abundant in the shallow south-eastern corner, which has long been a 'problem area' for nuisance macroalgae, and appear to have increased from post-Cut periods to the most recent period in both seasons (Figs \@ref(fig:macrophytes-pic8) and \@ref(fig:macrophytes-pic9)). Given this area is very shallow and close to the mouth of the Murray River, the recurrence of high green algal biomass is not surprising following seasonal increases in temperature (and shallow waters that are quick to warm) and, especially in spring, a new flush of nutrients from preceding winter river flows. Across the broader eastern Peel region, there has been a slight decline in spring algal biomass since the mid-1980s (Fig. \@ref(fig:macrophytes-pic6)), which could reflect both markedly lower river flows [\~50% drop in flow from the Murray sub-catchment since the 1970s, @valesini2019peelharvey] and greater tidal flushing.
:::
...
-   A broad program for future monitoring of the Peel-Harvey seagrass and macroalgal communities is proposed in Table \@ref(tab:macmon). *This program is purposely broad, outlining the main macrophyte components to be monitored and suggested methods*. Finer details for implementing this program will need to be determined, following clarity in level of interest in uptake and resourcing.



```{r macmon, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
library(readxl)
  options(knitr.kable.NA = "")
Diff_tab <- read_xlsx('tables/peel_tables.xlsx', sheet="macmon")
  kable(Diff_tab[,1:5],"html", escape = F, align = "c"
        , caption = "Proposed future monitoring regime for the macrophyte communities in the Peel-Harvey Estuary. Finer details on this broad approach will be tailored to uptake and resourcing.",bootstrap_options = "hover") %>%
  ...
  row_spec(5, background = "#D9D2C6", color = "black") %>%
  row_spec(6, background = "#FAF8F3", color = "gray") %>%
