...
People and estuaries are inextricably linked. These productive, safe and striking environments have long been attractive places to settle or visit due to the vast societal benefits they provide, such as food, protection, environmental buffering, lifestyles, livelihoods and connection to place. However, exploitation of their resources and the downstream impacts of over-development in their catchments has led to estuaries becoming among the most degraded of all aquatic ecosystems, which in turn threatens societal health [@thanh2021exam]. This close, reciprocal connection necessitates a socio-ecological view of these complex ecosystems [@thanh2020tracing], with the central question being *how do we best maintain the benefits provided by healthy estuaries, while also meeting the needs of our growing populations and supporting economic opportunities?* 

Successfully striking a balance or acceptable tradeoffs for the above question is a global issue, yet the pathways to achieving it are often poorly understood, rarely defined and seldom quantified [@heal2021water, @heal2022ensuring, @elliott2016eco]. In part, this reflects the complexities of both estuaries and societies across multiple scales. It also reflects the fact that management of socio-ecological systems requires long-term thinking, yet the future is uncertain and our ability to learn from hindsight is often hampered by patchy long-term observations [@polasky2011; @carpenter2015c]. One way of dealing with these challenges, and offering informed insights into the socio-ecological tradeoffs we will have to navigate in the foreseeable future, is the process of ‘scenario development’. Scenarios are plausible stories about how the future may unfold based on known patterns, anticipated drivers, or alternative societal choices. Scenarios aren’t predictions, but instead are alternative futures that explore particular trajectories of change and possibilities for adaptation [@raskin2005global; @mahmoud2009formal]. Scenario analysis connects *storylines* about the socio-ecological futures of most interest to stakeholders, with key *drivers* that ideally can be quantified (e.g., land-use mosaics, climate) and used as inputs for *predictive models* that capture the complexity of the human-natural system. This enables simulations about how the system is likely to *respond* from multiple perspectives and in turn assessments of the *trade-offs* between them, allowing decision-making to be tailored towards desired outcomes [elliott2019synthesis]. This general process of scenario analysis has been used to guide strategic planning at the catchment scale in various cases around the world, albeit with differing objectives, supporting drivers, assessment methods and assumptions [@carpenter2015c; @mahmoud2009formal; @bryan2011landscape; @booth2016from]. 

The Peel-Harvey Estuary and its surrounding community provides a rich opportunity to explore forward-looking scenarios about the sustainability of this complex system, and guide smarter management choices that support this fast-growing region yet minimise impacts on its largest natural asset. The catchment-estuary system has rich environmental and ecological historical data sets (in some cases dating back to the 1970s or earlier) that have been brought together with extensive field sampling under the this project (e.g., Chapters \@ref(model-hydrology), \@ref(macrophytes), \@ref(fish)). Further work within the project on local economic activity and the relative influence of external economic drivers supplements our socio-economic understanding of the Peel region (Chapter \@ref(econ)). Importantly, the coupled catchment-estuary model developed under other parts of the project (Chapters \@ref(model-hydrology) & \@ref(model-water-quality)) provides the framework for synthesising the biophysical complexity of the system, and translating scenario storylines (via underpinning drivers) into expected estuary health responses. A further key part of the opportunity provided by the Peel-Harvey is the interest and investment by a diverse range of stakeholders in the future wellbeing of the region, both from ecological and 
...
2) 2.	Translate each of the above key scenario narratives into input data and parameters that can be used by the coupled estuary-catchment model.
3) 3.	Forecast how key indices of both estuary and societal health are likely to respond under each 2050 scenario, relative to the present (‘base case’).
...
Diff_tab <- read_xlsx('tables/peel_tables_scen.xlsx', sheet="econind")
  kable(Diff_tab[,1:3],"html", escape = F, align = "c"
        , caption = "Summary of economic resilience indicators, and matching notation used in the regional econometrics model.",bootstrap_options = "hover")%>%
...
::: {.body-text width="100%"}
Socially, a Peel community that was environmentally-focused (e.g. ‘behaviour change towards environmentally sustainable practices’; ‘better community engagement/education in improving estuarine health’; ‘waterwise community’) and had good amenity to the estuary (e.g., ‘the estuary looks nice’; ‘good waterway recreation’; ‘good wildlife watching’) was highly valued (Fig. \@ref(fig:scenarios-pic3)a), with supporting highly-ranked goal statements (e.g. ‘mandatory water sensitive urban design’; ‘mandatory nutrient testing programs’; Fig. \@ref(fig:scenarios-pic3)b). From an economic perspective, better employment opportunities (e.g., ‘higher and/or more diverse employment’; ‘increased work skills’; ‘increased youth employment’) and sustaining key industries (e.g., ‘high growth/export industries’; ‘strong fishing/eco-tourism industries’) were clearly important (Fig. \@ref(fig:scenarios-pic3)a).
...
::: {.body-text width="100%"}
Table \@ref(tab:stories) summarises the main 2050 scenarios distilled from the above consultation and data analysis process, capturing the three main story groups, their broad underpinning catchment and climate drivers, and the anticipated responses of the socio-ecological health indices chosen to reflect the key stakeholder values and goals (Table \@ref(tab:shgoals)). 
:::


```{r stories, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
library(readxl)
  options(knitr.kable.NA = "")
Diff_tab <- read_xlsx('tables/peel_tables_scen.xlsx', sheet="stories")
  kable(Diff_tab[,1:4],"html", escape = F, align = "l"
        , caption = "Summary of the main 2050 scenarios prioritised by Peel stakeholders, including the broad underpinning catchment and climate drivers (see section 11.5.2 for more detail on quantifying these parameters), and the anticipated responses of the socio-ecological health indices chosen to reflect stakeholder values/goals.",bootstrap_options = "hover")%>%
   kable_styling(Diff_tab, bootstrap_options = "hover",
                full_width = T, position = "left",
                font_size = 11) %>% 
  kable_classic(full_width = T) %>%
  row_spec(0, background = "#5f543f", bold = TRUE, color = "white") %>%
  column_spec(1, width_max = "7em" ,color="black",bold = F) %>%
  column_spec(2, width_min = "15em" ,color="black") %>%
  column_spec(3, width_min = "15em" ,color="black") %>%
  footnote(alphabet = c("Stakeholder-selected management practices in their approximate order of importance. Note: while perennial pastures and irrigation management were also of interest to stakeholders, they could not be explicitly modelled due to a range of uncertainties","Additional management"))   %>%
  scroll_box(width = "40em",fixed_thead = FALSE)
...
::: {.body-text width="100%"}
The calibrated catchment model was used to explore each of the above three scenarios under two different climate conditions. It was also used to characterise the current or ‘base case’ catchment land-use and management level to provide a benchmark for comparison. This resulted in eight separate scenarios, which are summarised in Table \@ref(tab:scens). 


```{r scens, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
library(readxl)
  options(knitr.kable.NA = "")
Diff_tab <- read_xlsx('tables/peel_tables_scen.xlsx', sheet="scens")
  kable(Diff_tab[,1:4],"html", escape = F, align = "c"
        , caption = "Summary of the eight scenarios modelled using the calibrated catchment model. ‘Base case’ refers to current catchment land-use and management level.",bootstrap_options = "hover")%>%
   kable_styling(Diff_tab, bootstrap_options = "hover",
                full_width = T, position = "left",
                font_size = 11) %>% 
  kable_classic(full_width = T) %>%
  row_spec(0, background = "#5f543f", bold = TRUE, color = "white") %>%
  column_spec(1, width_max = "11em" ,color="black",bold = F) %>%
  column_spec(2, width_min = "3em" ,color="black",bold = T) %>%
  column_spec(3, width_min = "11em" ,color="black") %>%
  scroll_box(width = "40em",fixed_thead = FALSE)
...
**For the environmentally-optimistic scenario (1a and 1b)**, we assumed high phosphorus retentive fill would be used in all new urban developments. The urban footprint used was based on the planned urban development footprint (Fig. \@ref(fig:scenarios-pic6)) provided in the [Draft Green Growth Plan](https://espatial.planning.wa.gov.au/mapviewer/Index.html?viewer=greengrowthplan). The ‘extensive catchment management’ applied in this scenario included: 
:::
...
library(readxl)
  options(knitr.kable.NA = "")
Diff_tab <- read_xlsx('tables/peel_tables_scen.xlsx', sheet="loads")
  kable(Diff_tab[,1:8],"html", escape = F, align = "c"
        , caption = "Average annual flow, nutrient loads and flow-weighted concentrations of catchment inputs to the Peel-Harvey Estuary under each modelled scenario.",bootstrap_options = "hover")%>%
   kable_styling(Diff_tab, bootstrap_options = "hover",
                full_width = T, position = "left",
                font_size = 11) %>% 
  kable_classic(full_width = T) %>%
  row_spec(0, background = "#5f543f", bold = TRUE, color = "white") %>%
  column_spec(1, width_max = "11em" ,color="black",bold = F) %>%
  column_spec(2, width_min = "5em" ,color="black") %>%
  column_spec(3, width_min = "5em" ,color="black",bold = T) %>%
  scroll_box(width = "40em",fixed_thead = FALSE)
...
::: {.body-text width="100%"}
The water and nutrient outputs from the catchment modelling created a characteristic set of estuary water quality and ecosystem indicators for each scenario. These are compared side-by-side in Figures \@ref(fig:scenarios-pic8)-\@ref(fig:scenarios-pic17) by looking at scenario predictions relative to the base-case (0a). The various indicators show a complex pattern of change due to: a) the changing climate (“a” vs “b”), and b) the changing land-use/land management scenarios (“0”, “1”, “2” and “3”).  
...
For example, DO in shallow areas of the main lagoons is predicted to improve, whilst the rivers show a considerable decline, particularly the Murray River which is on average expected to be >2 mg/L lower than present. Nutrients were also varied across the domain, with slight reductions in some regions of the lagoons, but increases in the rivers that varied in accordance with the nutrient loading scenario (Fig. \@ref(fig:scenarios-pic12)). 

```{r scenarios-pic8, echo = FALSE, out.width='100%', class = "text-image", fig.cap = "Comparison of winter water age difference (∆τ̅) (days) between the scenarios and the reference (base) case simulation 0a. A positive change indicates the scenario has older water than the base-case."}
knitr::include_graphics("images/scenarios/picture8.png")
```

```{r scenarios-pic9, echo = FALSE, out.width='100%', class = "text-image", fig.cap = "Comparison of winter salinity differences (∆S)̅ (psu) between the scenarios and the reference (base) case simulation 0a. A positive change indicates a relatively more saline scenario."}
knitr::include_graphics("images/scenarios/picture9.png")
```

```{r scenarios-pic10, echo = FALSE, out.width='100%', class = "text-image", fig.cap = "Comparison of winter bottom oxygen differences (∆DO) ̅ (mg/L) between the scenarios and the reference (base) case simulation 0a. A positive change indicates a scenario with more oxygen than the base-case."}
knitr::include_graphics("images/scenarios/picture10.png")
```

```{r scenarios-pic11, echo = FALSE, out.width='100%', class = "text-image", fig.cap = "Comparison of winter bottom oxygen differences (∆DO) ̅_hyp (-) between the scenarios and the reference (base) case simulation 0a. A positive change indicates a scenario a higher probability of hypoxia than the base-case."}
knitr::include_graphics("images/scenarios/picture11.png")
```

```{r scenarios-pic12, echo = FALSE, out.width='100%', class = "text-image", fig.cap = "Comparison of winter bottom TN differences (∆TN) ̅ (mg/L) between the scenarios and the reference (base) case simulation 0a. A positive change indicates a scenario with more nitrogen than the base-case."}
knitr::include_graphics("images/scenarios/picture12.png")
```

```{r scenarios-pic13, echo = FALSE, out.width='100%', class = "text-image", fig.cap = "Comparison of winter bottom TP differences (∆TP) ̅ (mg/L) between the scenarios and the reference (base) case simulation 0a. A positive change indicates a scenario with more phosphorus than the base-case."}
knitr::include_graphics("images/scenarios/picture13.png")
```

```{r scenarios-pic14, echo = FALSE, out.width='100%', class = "text-image", fig.cap = "Comparison of winter total chlorophyll-a differences (∆TCHLA) ̅ (g/L) between the scenarios and the reference (base) case simulation 0a. A positive change indicates a scenario with more chlorophyll-a than the base-case."}
knitr::include_graphics("images/scenarios/picture14.png")
```

```{r scenarios-pic15, echo = FALSE, out.width='100%', class = "text-image", fig.cap = "Comparison of dinoflagellate bloom likelihood differences (∆HAB) ̅_dino  (-) between the scenarios and the reference (base) case simulation 0a. A positive change indicates a scenario with more bloom likelihood than the base-case."}
knitr::include_graphics("images/scenarios/picture15.png")
```

```{r scenarios-pic16, echo = FALSE, out.width='100%', class = "text-image", fig.cap = "Comparison of adult crab index differences ∆CHI~juv~ (-) between the scenarios and the reference (base) case simulation 0a. A positive change indicates a scenario with improved habitat condition relative to the base-case"}
knitr::include_graphics("images/scenarios/picture16.png")
```

```{r scenarios-pic17, echo = FALSE, out.width='100%', class = "text-image", fig.cap = "Comparison of adult crab index differences ∆CHI~adu~ (-) between the scenarios and the reference (base) case simulation 0a. A positive change indicates a scenario with improved habitat condition relative to the base-case."}
knitr::include_graphics("images/scenarios/picture17.png")
```

The variation of the key water quality attributes and ecological habitats in the scenarios are further summarized in Figures \@ref(fig:scenarios-pic18)-\@ref(fig:scenarios-pic21), in comparison with the outputs in the historical years, by reducing the indicators to a single annual value. The rivers (Figure \@ref(fig:scenarios-pic18)-\@ref(fig:scenarios-pic19)) show higher salinity, water age and phytoplankton biomass, and lower DO in the “b” scenarios while the changes between the land management scenarios are relatively smaller. In comparison to the historical years, the “b” scenarios with dry climate is shown to have lower the IWQ, turbidity, and increased ecological habitat areas for crab and phytoplankton in the Serpentine River. Whilst in the “a” scenarios, these attributes are in comparable ranges with the historical years. The land management is shown to have a strong impact on the nutrient levels of the Serpentine River, where the mean TN and TP concentration are much higher in the “2” and “3” scenarios (with relatively higher concentration nutrient loading from catchment) than the “0” and “1” scenarios. The future scenarios favor the growth condition for cyanobacteria, and the “2” and “3” scenarios favor the growth condition for dinoflagellate.  Whilst in the Murray River, the “a” scenarios show more variation in the nutrient levels, although the nutrient concentrations in the “b” scenarios are relatively higher than the “a” scenarios. The habitat area stays stable for cyanobacteria, but improves for dinoflagellate. The habitat area for juvenile in the “b” scenarios are relatively lower to the “a” scenarios, but for adult crab are higher.  

<br>

```{r scenarios-pic18, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "Scenario comparison with historical conditions within the *Serpentine River*, for the averaged water quality metrics (top six panels) and ecosystem metrics (bottom six panels)."}
knitr::include_graphics(c("images/scenarios/picture18a.png","images/scenarios/picture18b.png"))
```

```{r scenarios-pic19, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "Scenario comparison with historical conditions within the *Murray River*, for the averaged water quality metrics (top six panels) and ecosystem metrics (bottom six panels)."}
knitr::include_graphics(c("images/scenarios/picture19a.png","images/scenarios/picture19b.png"))
```

```{r scenarios-pic20, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "Scenario comparison with historical conditions within the *Harvey Lagoon*, for the averaged water quality metrics (top six panels) and ecosystem metrics (bottom six panels)."}
knitr::include_graphics(c("images/scenarios/picture20a.png","images/scenarios/picture20b.png"))
```

```{r scenarios-pic21, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "Scenario comparison with historical conditions within the *Peel Inlet*, for the averaged water quality metrics (top six panels) and ecosystem metrics (bottom six panels)."}
knitr::include_graphics(c("images/scenarios/picture21a.png","images/scenarios/picture21b.png"))
```

<br>

The lagoons illustrate different behaviour to the rivers in the scenarios (Figs. \@ref(fig:scenarios-pic20)-\@ref(fig:scenarios-pic21)). Although the IWQ still decreases in the scenarios when compared to historical years, the habitat area for cyanobacteria has dropped to nearly zero, while the habitat area for dinoflagellate stays relatively stable. The dry climate in the “b” scenarios is shown to lower the juvenile and dinoflagellate habitat areas but increase the adult crab habitat area. The “b” scenario has more impacts on the Harvey Lagoon than the Peel Inlet in terms of crab and dinoflagellate habitat areas. Whilst in the Peel Inlet, the differences of the habitat areas between the “a” and “b” scenarios are relatively smaller, indicating the Peel Inlet is relatively insensitive to the climate change. The mean DO in the lagoons has improved although the nutrient levels are still high in the scenarios. 

The total amount of nutrient loading and export in the scenarios are relatively small compared to the historical years, yet the ratios of nutrient retention to the catchment loading show similar trends to the historical years in the post-Cut period (Fig. \@ref(fig:scenarios-pic22)-\@ref(fig:scenarios-pic23)). The “b” scenarios have less nutrient loading when compared to the “a” scenarios and the historical years due to the drier climate. The estuary is shown to net export the nutrients in these scenarios when the ocean flushing exceeds the catchment inputs. 
:::

```{r scenarios-pic22, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "Total retention of nitrogen (a) and phosphorus (b) for historical years and the various scenarios."}
knitr::include_graphics(c("images/scenarios/picture22.png"))
```

```{r scenarios-pic23, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "Scenario comparison with historical conditions in terms of nitrogen (top) and phosphorus (bottom) retention."}
knitr::include_graphics(c("images/scenarios/picture23.png"))
```
...
::: {.body-text width="100%"}
The dynamics and drivers of local competitiveness and comparative advantage shape a regional economy’s responsiveness to externalities (i.e., resilience), and can help explain the underlying forces leading to ‘*catching-up*’, ‘*falling behind*’ and ‘*forging ahead*’ trajectories (Chapter \@ref(econ)). The three main scenarios reported in the previous sections (1, 2, 3) have been considered through this lens, and according to the analysis framework previously reported.  

An example of the assessment of economic indicators for each scenario is described in Tables \@ref(tab:econscen1)–\@ref(tab:econscen3). 
:::


```{r econscen1, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
library(readxl)
  options(knitr.kable.NA = "")
Diff_tab <- read_xlsx('tables/peel_tables_scen.xlsx', sheet="econscen1")
  kable(Diff_tab[,1:5],"html", escape = F, align = "c"
        , caption = "Economic indicator assessment for **Scenario 1**.",bootstrap_options = "hover")%>%
   kable_styling(Diff_tab, bootstrap_options = "hover",
                full_width = T, position = "left",
                font_size = 11) %>% 
  kable_classic(full_width = T) %>%
  add_header_above(c("Scenario 1: Modest urban expansion, focus on land management innovation and environmental stewardship ^a^" = 5),font_size = 14,bold = TRUE) %>%
  row_spec(0, background = "#5f543f", bold = TRUE, color = "white") %>%
  column_spec(1, width_max = "11em" ,color="black",bold = T) %>%
  column_spec(2, width_min = "5em" ,color="black") %>%
  column_spec(4, width_min = "5em" ,color="black",bold = T) %>%
  footnote(alphabet = c("Under this scenario, more people would be living in the region (e.g. Perth-Peel 3.5 mil). Employment in supporting industries related to catchment management (revegetation, innovative remediation of soils and better agricultural management). Growth in urban areas will create demand in other areas like housing (building new urban dwellings), and also industry. Existing mining activity would be maintained. The area would seek to improve desirability of the region and encourage skilled workers, by adopting a regional focus on education, fostering cultural activities, liveability initiatives and eco-tourism opportunities. In summary:

•	SAPPR’ development footprint 
o	Planned urban development (Fig. 11.5 without additional urban)
o	Planned industrial development, concentrated around mining activities 
(e.g. Peel Business Park(?)) 

•	Community building strategies and liveability enhancements
o	Initiatives to develop improved higher education facilities
o	Environmentally sustainable urban design (urban bushland and greening and water sensitive urban design)
o	Community stewardship, arts & culture focus

•	No further intensive agricultural development, but refinement
o	Increased technology adoption
o	New agribusiness opportunities (incl carbon farming, smart agriculture innovation)

•	Increased eco-tourism 
o	(eg Tourism and Visitation plan by Shire of Murray)

•	Climate change: 
o	Influence on economy due to climate change events (e.g. less water, more extreme events, flooding and storm surges). 
o	Local competitive advantage under climate change – coastal (water) living, but more floods and reduced water quality.
...
```{r econscen2, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
library(readxl)
  options(knitr.kable.NA = "")
Diff_tab <- read_xlsx('tables/peel_tables_scen.xlsx', sheet="econscen2")
  kable(Diff_tab[,1:5],"html", escape = F, align = "c"
        , caption = "Economic indicator assessment for **Scenario 2**.",bootstrap_options = "hover")%>%
   kable_styling(Diff_tab, bootstrap_options = "hover",
                full_width = T, position = "left",
                font_size = 11) %>% 
  kable_classic(full_width = T) %>%
  add_header_above(c("Scenario 2: Modest urban expansion, limited land management and food export focus ^a^" = 5),font_size = 14,bold = TRUE) %>%
  row_spec(0, background = "#5f543f", bold = TRUE, color = "white") %>%
  column_spec(1, width_max = "11em" ,color="black",bold = T) %>%
  column_spec(2, width_min = "5em" ,color="black") %>%
  column_spec(4, width_min = "5em" ,color="black",bold = T) %>%
  footnote(alphabet = c("Under this scenario, more people would be living in the region (e.g. Perth-Peel 3.5 mil). Likely growth would be in areas like housing (building new urban dwellings), mining and industry, and intensive horticulture. 
...
```{r econscen3, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
library(readxl)
  options(knitr.kable.NA = "")
Diff_tab <- read_xlsx('tables/peel_tables_scen.xlsx', sheet="econscen3")
  kable(Diff_tab[,1:5],"html", escape = F, align = "c"
        , caption = "Economic indicator assessment for **Scenario 3**.",bootstrap_options = "hover")%>%
   kable_styling(Diff_tab, bootstrap_options = "hover",
                full_width = T, position = "left",
                font_size = 11) %>% 
  kable_classic(full_width = T) %>%
  add_header_above(c("Scenario 3: Extensive urban expansion, limited land management and food export focus ^a^" = 5),font_size = 14,bold = TRUE) %>%
  row_spec(0, background = "#5f543f", bold = TRUE, color = "white") %>%
  column_spec(1, width_max = "11em" ,color="black",bold = T) %>%
  column_spec(2, width_min = "5em" ,color="black") %>%
  column_spec(4, width_min = "5em" ,color="black",bold = T) %>%
  footnote(alphabet = c("Under this scenario, more people would be living in the region (e.g. Perth-Peel 3.5 mil). Likely growth would be in areas like housing (building new urban dwellings), mining and industry, and intensive horticulture. For example:
...
```{r scenarios-pic24, echo = FALSE, out.width='55%', fig.show='hold', class = "text-image", fig.cap = "Decision support logic for scenario testing."}
knitr::include_graphics(c("images/scenarios/picture24.png"))
```

::: {.body-text width="100%"}
For this purpose, we begin by adopting a modified Driver-Pressure-State-Impact-Response (DPSIR) coastal management framework as the basis for linking drivers, actions and responses to the state of system [@elliott2017and]. Specifically, the more comprehensive DAPSI(W)R(M) framework (hereafter DAPSIWRM) was employed and configured for the Peel-Harvey catchment-estuary system to structure the effects of changes to the natural system on the varied human uses and benefits of the marine system, and vice-versa. The utility of this approach is its ability to combine various exogenous (external) and endogenous (internal) drivers across multiple scales, in a way that clearly depicts the feedbacks and inter-relationships that develop in a what is a complex Socio-Ecological-System (SES), structured using an established logic. For the Peel-Harvey catchment-estuary system feedbacks and linkages occur between environmental quality and economy, though highlighting that not all economic growth in the region is directly connected to the environment directly (Fig. \@ref(fig:scenarios-pic25)). 

```{r scenarios-pic25, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "An example schematic of the DAPSIWRM framework applied to the Peel-Harvey catchment-estuary system, depicting how drivers, actions and pressures create environmental and economic impacts that can ultimately drive socio-environmental change, potentially transitioning to a new state."}
knitr::include_graphics(c("images/scenarios/picture25.png"))
```
...
Therefore, to operationalise the framework as a decision support tool we introduce a modified Bow-Tie method [@cormier2019]. The core structure of the framework is presented generically below in Figure \@ref(fig:scenarios-pic26). Traditionally, the Bow-Tie method is used for risk analysis to understand how changes or actions that may create risk could be isolated, controlled and eliminated. This logic makes sense in the context of coastal management as we ultimately seek to eliminate risks to the environment (e.g., poor water quality) or risks to the community (e.g., industry decline and job loss). The approach presented in @cormier2019 is an interesting option as takes an ISO-compliant risk management framework to coastal ecosystem decline, and at its core is therefore designed from the point of view of supporting managers to make the right decision to reduce risk to an asset in the face of “escalating” factors – in our case the drivers changing the system. This method is however relatively new in this context and there are limited examples how this can integrate risks to the environment and economy in a tractable way. Further, it has been identified that rather than aiming for sustainable management to reduce risk, we could consider from the counter point of view, which is to maximise opportunity (e.g., for economic growth). In essence, at the core of our decision making to balance trade-offs, is a desire to a) reduce risk to vulnerable assets (e.g., the environment) and enhance opportunity to improve regional competitiveness (e.g., through job growth). Therefore, in our context, we have developed a revised Bow-Tie decision support framework that is able to highlight how escalating factors (drivers and actions) are either reducing our ability to realise an opportunity, or creating vulnerability in a valued asset, and therefore need to be managed. Management of the system (right-side of the Bow-Tie) is supported through undertaking adaptive measures and tracking progress against environmental and economic indicators that are known to adequately describe system function from the decision-makers point of view (Fig. \@ref(fig:scenarios-pic26)). 

```{r scenarios-pic26, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "Modified Bow-Tie framework for managing towards successful adaptation, linking essential elements of the DAPSIWRM framework into a control logic. Note the classification of each Bow-Tie step to the relevant DAPSIWRM component, indicated on the bottom."}
knitr::include_graphics(c("images/scenarios/picture26.png"))
```

The structure presented above is generic, but can be tailored to specific “assets” and/or specific escalating factors (e.g., how is climate change escalating risks associated with hypoxia?). This way a standard logic can be used to assess the varied stressors on the system. However, inherently the Peel-Harvey catchment-estuary system is inherently complex facing multiple stressors, all impacting on a wide array of system attributes of relevance to management. We therefore applied the general framework in Figure \@ref(fig:scenarios-pic26) to align it with the varied issues and indicators raised by the project team and stakeholders in Table 2. The diagram exemplifies how the wide range of policies, plans and actions being undertaken by local agencies can be structured in a standard way.  This can ensure consistency between management plans identified at the project or organisation level, whilst encouraging a common approach to risk and opportunity identification, and how they are monitored, evaluated and reported. We also highlight that, whilst we have aimed to be as comprehensive in coverage of this framework as possible, the framework presented here may require further refinement and inclusion as it becomes operationalised. A process of auditing relevant LGA policies and management plans relevant to Peel-Harvey catchment-estuary system is therefore recommended to allow for continual revision of framework.  

```{r scenarios-pic27, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "Structuring of relevant drivers, actions, pressures and responses identified in the Peel-Harvey stakeholder consultation into the Bow-Tie operational control structure."}
knitr::include_graphics(c("images/scenarios/picture27.png"))
```

As a demonstration of the decision-support framework for the specific scenarios identified during our stakeholder consultation process, and the subsequent analysis in section 11.5.1, we visually depict the scenarios 1, 2 and 3 in Figures  \@ref(fig:scenarios-pic28) -  \@ref(fig:scenarios-pic30), by linking the identified drivers and management measures with the reported environmental and economic indicators. The results demonstrate that the environmentally-optimistic scenario can provide the best option for growth through encouraging diversification in export oriented industries and improving the specialisation of agriculture. 
:::

```{r scenarios-pic28, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "Scenario 1 drivers, adaptive measures and assessment of indicators for 1a (top) and 1b (bottom)."}
knitr::include_graphics(c("images/scenarios/picture28a.png","images/scenarios/picture28b.png"))
```

```{r scenarios-pic29, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "Scenario 2 drivers, adaptive measures and assessment of indicators for 2a (top) and 2b (bottom)."}
knitr::include_graphics(c("images/scenarios/picture29a.png","images/scenarios/picture29b.png"))
```

```{r scenarios-pic30, echo = FALSE, out.width='100%', fig.show='hold', class = "text-image", fig.cap = "Scenario 3 drivers, adaptive measures and assessment of indicators for 3a (top) and 3b (bottom)."}
knitr::include_graphics(c("images/scenarios/picture30a.png","images/scenarios/picture30b.png"))
```
...
::: {.body-text width="100%"}
Data used for the model and supporting the conclusions of this study are available in the Peel ARC Linkage Project Github repository [@busch2023]. This includes, the full set of model input files of the Peel-Harvey Estuary Response Model which are archived in the [PHERM model repository folder](https://github.com/AquaticEcoDynamics/Peel_ARC/tree/master/Models/PHERM). To run the simulation requires the AED software package [@hipsey2022], and the model TUFLOW-FV, available from @wbm2023tuflowb. 
...
::: {.author-info width="100%"}
Cover image courtesy of *Peel-Harvey Catchment Council*.
:::
